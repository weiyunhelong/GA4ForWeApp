...
function GA4MPClient(apiSecret, measurementId, clientId) {
  ...
  this.send = function (events, postpone = false) {
    ...
    if (postpone) {
      ...
      if (eventMax) {
        ...
        //WxRequest(this.DEBUG_ENDPOINT, JSON.stringify({
        WxRequest(this.GA_ENDPOINT, JSON.stringify({
          client_id: this.CLIENT_ID,
          timestamp_micros:new Date().getTime()*1000,
          non_personalized_ads:false,
          events: events
        })).then(res => {
          //console.error('GA4', JSON.stringify(events));
        }).catch(error => {
          console.log(error);
        });
      }
...
function createClient(apiSecret, measurementId, clientId) {
